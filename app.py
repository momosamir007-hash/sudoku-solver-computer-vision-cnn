...
try:
    ...
    st.sidebar.error("⚠️ لم يتم العثور على مفتاح Gemini API في إعدادات الأمان (Secrets). يرجى إضافته لتعمل طبقة التحقق الثانية.")
    gemini_api_key = None
...
def predict_with_confidence(model, cells, device, confidence_threshold=0.70):
    """توقع الأرقام محلياً مع قص الحواف لتنظيف التشويش وحساب نسبة الثقة."""
    grid = []
    ...
    with torch.no_grad():
        for i, cell in enumerate(cells):
            ...
            clean_cell = cv2.resize(cropped_cell, (w, h), interpolation=cv2.INTER_AREA)
            
            # حل مشكلة القنوات (تحويل 1 Channel إلى 3 Channels)
            tensor_cell = torch.FloatTensor(clean_cell).unsqueeze(0).unsqueeze(0).repeat(1, 3, 1, 1).to(device)
            ...
            # إذا كانت الثقة أقل من الحد، سجلها للمراجعة (ونمرر الخلية النظيفة)
            if confidence < confidence_threshold:
                low_confidence_cells.append({
                    'index': i, 'row': row, 'col': col, 
                    'digit': digit, 'confidence': confidence, 'image': clean_cell
                })
...
def validate_cell_with_gemini(cell_image, api_key):
    """تكبير الخلية النظيفة ثم إرسالها إلى Gemini للتحقق منها بدقة عالية."""
    # الصورة نظيفة بالفعل من الدالة السابقة، نحتاج فقط لتكبيرها ليقرأها VLM بوضوح
    enlarged_cell = cv2.resize(cell_image, (150, 150), interpolation=cv2.INTER_LANCZOS4)
...
if uploaded_file is not None:
    ...
    if st.button("استخراج الشبكة والتحقق"):
        with st.spinner("جاري قص الخلايا، تحليل الأرقام، والتحقق..."):
            try:
                ...
                if cells is None:
                    ...
                    # 3. التحقق التلقائي باستخدام Gemini
                    if gemini_api_key and low_conf_cells:
                        st.info(f"تم العثور على {len(low_conf_cells)} خلايا تحتاج لتأكيد. جاري التحقق باستخدام Gemini...")
